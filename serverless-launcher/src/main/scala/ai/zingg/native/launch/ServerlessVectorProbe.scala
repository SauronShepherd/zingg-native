...
object ServerlessVectorProbe {
  def run(spark: SparkSession): Unit = {
    ...
    val provider =
      NativeOperationProvider.fromSpark(spark, "model.vector-differential")
    val actual = provider
      .vectorValue(input, "vector", "value")
      .select("id", "value")
      .orderBy("id")
      .collect()
    require(actual.length == 4, s"Vector probe returned ${actual.length} rows")
    require(
      actual(0).getDouble(1) == 1.5d,
      s"Dense vector mismatch: ${actual(0)}"
    )
    require(
      actual(1).getDouble(1) == 0.2d,
      s"Sparse-present vector mismatch: ${actual(1)}"
    )
    require(
      actual(2).getDouble(1) == 0.0d,
      s"Sparse-missing vector mismatch: ${actual(2)}"
    )
    require(actual(3).isNullAt(1), s"Null vector mismatch: ${actual(3)}")
    println(
      "NATIVE_VECTOR_DIFFERENTIAL_PASS dense=1.5 sparsePresent=0.2 sparseMissing=0.0 null=true rows=4"
    )
  }
}
