...
object ServerlessNumericDifferentialProbe {
  ...
  private val Rules = Seq(
    Rule(
      ...
      Seq(
        (null, null),
        (Integer.valueOf(Int.MinValue), Integer.valueOf(Int.MinValue)),
        (Integer.valueOf(-1), Integer.valueOf(1)),
        (Integer.valueOf(0), Integer.valueOf(1)),
        (Integer.valueOf(Int.MaxValue), Integer.valueOf(Int.MaxValue - 1))
      ),
    ...
    Rule(
      ...
      Seq(
        ...
        (
          ...
          java.lang.Long.valueOf(Long.MaxValue - 1L)
        )
      ),
      DataTypes.LongType
    ),
    ...
    Rule(
      ...
      Seq(
        ...
        (
          java.lang.Float.valueOf(Float.NaN),
          java.lang.Float.valueOf(Float.NaN)
        ),
        (
          java.lang.Float.valueOf(Float.PositiveInfinity),
          java.lang.Float.valueOf(Float.PositiveInfinity)
        )
      ),
      DataTypes.FloatType
  ...
  def run(spark: SparkSession): Unit = {
    val provider =
      NativeOperationProvider.fromSpark(spark, "numeric-differential")
    Rules.foreach { rule =>
      ...
      rule.values.foreach { case (left, right) =>
        rows.add(RowFactory.create(left, right))
      }
      ...
      val oracleClass = Class.forName(
        s"zingg.common.core.similarity.function.${rule.operation}"
      )
      val oracle = oracleClass.getDeclaredConstructor().newInstance()
      val call = oracleClass.getMethods
        .find(m => m.getName == "call" && m.getParameterCount == 2)
        .get
      val expected = rule.values.map { case (left, right) =>
        call
          .invoke(oracle, left, right)
          .asInstanceOf[java.lang.Double]
          .doubleValue()
      }
      actual.zip(expected).zipWithIndex.foreach {
        case ((nativeValue, referenceValue), index) =>
          require(
            (nativeValue.isNaN && referenceValue.isNaN) ||
              math.abs(nativeValue - referenceValue) <= 1e-9,
            s"Numeric differential mismatch rule=${rule.operation} row=$index native=$nativeValue oracle=$referenceValue"
          )
      }
      println(
        s"NATIVE_NUMERIC_DIFFERENTIAL_PASS rule=${rule.operation} rows=${actual.length}"
      )
    }
    println(
      s"NATIVE_NUMERIC_DIFFERENTIAL_SUMMARY rules=${Rules.size} rows=${Rules.map(_.values.size).sum}"
    )
  }
}
