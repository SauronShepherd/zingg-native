...
object ServerlessDateArrayDifferentialProbe {
  ...
  def run(spark: SparkSession): Unit = {
    val provider =
      NativeOperationProvider.fromSpark(spark, "date-array-differential")
    ...
    val dateRows = new ArrayList[Row]();
    dates.foreach { case (l, r) => dateRows.add(RowFactory.create(l, r)) }
    val dateClass = Class.forName(
      "zingg.common.core.similarity.function.DateSimilarityFunction"
    )
    val dateOracle = dateClass.getDeclaredConstructor().newInstance()
    val dateCall = dateClass.getMethod(
      "call",
    ...
    try {
      zones.foreach { zone =>
        ...
        val dateActual = provider
          .similarityByZinggName(
            dateInput,
          ...
          .select("actual")
          .collect()
          .map(_.getAs[java.lang.Double](0).doubleValue())
        expectedDates.zip(dateActual).foreach { case (expected, actual) =>
          require(
            same(actual, expected),
            s"Date differential mismatch zone=$zone native=$actual oracle=$expected"
          )
        }
      }
    } finally {
    ...
    println(
      s"NATIVE_TYPED_DIFFERENTIAL_PASS rule=DateSimilarityFunction rows=${dates.size} zones=${zones.size}"
    )
    ...
    val arraySchema = StructType(
      Seq(
        StructField(
          "left_value",
          ArrayType(DataTypes.DoubleType, containsNull = true),
          nullable = true
        ),
        StructField(
          "right_value",
    ...
    val arrays = Seq[(Array[java.lang.Double], Array[java.lang.Double])](
      (null, null),
      (
        Array(java.lang.Double.valueOf(1.0d), java.lang.Double.valueOf(0.0d)),
        Array(java.lang.Double.valueOf(1.0d), java.lang.Double.valueOf(0.0d))
      ),
      (
        Array(java.lang.Double.valueOf(1.0d), java.lang.Double.valueOf(2.0d)),
        Array(java.lang.Double.valueOf(2.0d), java.lang.Double.valueOf(1.0d))
      ),
      (
    ...
    arrays.foreach { case (l, r) => arrayRows.add(RowFactory.create(l, r)) }
    val arrayInput = spark.createDataFrame(arrayRows, arraySchema)
    val arrayActual = provider
      .similarityByZinggName(
        arrayInput,
      ...
      .collect()
      .map(_.getAs[java.lang.Double](0).doubleValue())
    val arrayClass = Class.forName(
      "zingg.common.core.similarity.function.ArrayDoubleSimilarityFunction"
    )
    val arrayOracle = arrayClass.getDeclaredConstructor().newInstance()
    val arrayCall = arrayClass.getMethod(
      "call",
      classOf[Array[java.lang.Double]],
      classOf[Array[java.lang.Double]]
    )
    arrays.zip(arrayActual).foreach { case ((left, right), actual) =>
      val expected = arrayCall
        .invoke(arrayOracle, left, right)
        .asInstanceOf[java.lang.Double]
        .doubleValue()
      require(
        same(actual, expected),
        s"Array differential mismatch native=$actual oracle=$expected"
      )
    }
    println(
      s"NATIVE_TYPED_DIFFERENTIAL_PASS rule=ArrayDoubleSimilarityFunction rows=${arrays.size}"
    )
    println(
      s"NATIVE_TYPED_DIFFERENTIAL_SUMMARY rules=2 rows=${dates.size + arrays.size}"
    )
  }
}
