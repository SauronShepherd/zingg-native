...
object NativeExpressions {
  ...
  def jaro(left: Column, right: Column): Column = {
    ...
    val initialMatches = struct(emptyIntArray.alias("used"),emptyIntArray.alias("sources"),emptyIntArray.alias("targets"))
    val matches = aggregate(indexArray(size(chars1)), initialMatches, (state, i) => {
      ...
      val candidates = transform(filter(indexArray(windowLength), offset => {
        val j = start + offset
        element_at(chars2, j + lit(1)) === sourceValue && !exists(state.getField("used"), used => used === j)
      }), offset => start + offset)
      val candidate = try_element_at(candidates, lit(1))
      when(candidate.isNotNull,struct(concat(state.getField("used"),array(candidate)).alias("used"),concat(state.getField("sources"),array(i)).alias("sources"),concat(state.getField("targets"),array(candidate)).alias("targets"))).otherwise(state)
    })
    ...
    val score = (count.cast("double")/n.cast("double") + count.cast("double")/m.cast("double") +(count.cast("double")-transpositions)/count.cast("double")) / lit(3.0)
  ...
  private def lowerAffineUnit(unit: Column): Column = {
    ...
    when(c >= 0xD800 && c <= 0xDFFF, unit).otherwise(utf16Hex(lower(decodeUtf16Hex(unit))))
  }
  ...
  private def affineCharScore(aUnit: Column, bUnit: Column): Column = {
    ...
    val approximateGroups = Seq("dt","gj","rl","mn","pbv","aeuio",",.").map(_.toCharArray.map(ch => f"${ch.toInt}%04X").toVector)
  ...
  def affineGap(left:Column,right:Column):Column={
    ...
    val rows=aggregate(indexArray(n, oneBased = true),initial,(outer,i)=>{
      ...
      val row=aggregate(indexArray(m, oneBased = true),innerInit,(state,j)=>{
        ...
        struct(concat(state.getField("mrow"),array(mv)).alias("mrow"),concat(state.getField("srow"),array(sv)).alias("srow"),concat(state.getField("trow"),array(tv)).alias("trow"),greatest(state.getField("best"),mv,sv,tv).alias("best"))
      })
  ...
  def email(left:Column,right:Column):Column={def local(c:Column)=element_at(split(c.cast("string"),"@",-1),1);stringBase(left,right){affineGap(local(left),local(right))}}
  def pin(left:Column,right:Column):Column={def part(c:Column)=element_at(split(c.cast("string"),"-",-1),1);stringBase(left,right){when(part(left)===part(right),lit(1.0)).otherwise(lit(0.0))}}
  def onlyAlphaExact(left:Column,right:Column):Column=stringBase(left,right){when(regexp_replace(left.cast("string"),"[0-9.]","")===regexp_replace(right.cast("string"),"[0-9.]",""),lit(1.0)).otherwise(lit(0.0))}
  def onlyAlphaAffine(left:Column,right:Column):Column=stringBase(left,right){affineGap(regexp_replace(left.cast("string"),"[0-9.]",""),regexp_replace(right.cast("string"),"[0-9.]",""))}
  def sameFirstWord(left:Column,right:Column):Column=stringBase(left,right){affineGap(element_at(split(left.cast("string"),"-",-1),1),element_at(split(right.cast("string"),"-",-1),1))}

  def javaLastWord(value:Column):Column={val s=value.cast("string"); val withoutTrailing=regexp_replace(s," +$","");when(s.isNull,s).when(length(s)===0,s).when(length(withoutTrailing)===0,lit(null).cast("string")).otherwise(element_at(split(withoutTrailing," ",-1),-1))}
  ...
  def truncate(value:Column,places:Int,dataType:String):Column={
    ...
    when(value.isNull,value).otherwise((when(scaled>=0.0,floor(scaled)).otherwise(ceil(scaled))/scale).cast(dataType))
  }
  def trimDigits(value:Column,digits:Int,dataType:String):Column={
    val scale=BigDecimal(10).pow(digits)
    if(dataType=="int"||dataType=="long") {val q=value.cast("decimal(38,0)")/lit(scale);when(value.isNull,value).otherwise(when(q>=0,floor(q)).otherwise(ceil(q)).cast(dataType))}
  ...
  def javaRound(value:Column):Column={val x=value.cast("double");when(x.isNull,lit(null).cast("long")).when(isnan(x),lit(0L)).when(x >= lit(Long.MaxValue.toDouble),lit(Long.MaxValue)).when(x <= lit(Long.MinValue.toDouble),lit(Long.MinValue)).otherwise(floor(x+0.5).cast("long"))}
  ...
  def vectorValue(value:Column):Column={
    val vectorType=value.getField("type");val size=value.getField("size");val indices=value.getField("indices");val values=value.getField("values")
    val dense=try_element_at(values,lit(3))
    val sparse=aggregate(sequence(lit(1),size),lit(0.0),(acc,pos)=>when(try_element_at(indices,pos.cast("int"))===lit(2),try_element_at(values,pos.cast("int"))).otherwise(acc))
...
object PublicRewriteRules {
  ...
  private val similarities:Seq[RewriteRule]=Seq(
    binary("SimilarityFunctionExact","rewrite.similarity.exact")(exact),binary("StringSimilarityFunction","rewrite.similarity.string_base")((l,r)=>stringBase(l,r){lit(0.0)}),binary("CheckNullFunction","rewrite.similarity.check_null")(checkNull),binary("CheckBlankOrNullFunction","rewrite.similarity.check_blank_or_null")(checkBlankOrNull),binary("IntegerSimilarityFunction","rewrite.similarity.integer")(integerSimilarity),binary("LongSimilarityFunction","rewrite.similarity.long")(longSimilarity),binary("DoubleSimilarityFunction","rewrite.similarity.double")((l,r)=>floatingSimilarity(l,r,"double")),binary("FloatSimilarityFunction","rewrite.similarity.float")((l,r)=>floatingSimilarity(l,r,"float")),binary("DateSimilarityFunction","rewrite.similarity.date")(dateSimilarity),binary("ArrayDoubleSimilarityFunction","rewrite.similarity.array_double")(arrayDoubleSimilarity),binary("JaccSimFunction","rewrite.similarity.jaccard")(jaccard),binary("NumbersJaccardFunction","rewrite.similarity.numbers_jaccard")(numbersJaccard),binary("ProductCodeFunction","rewrite.similarity.product_code")(productCode),binary("JaroWinklerFunction","rewrite.similarity.jaro")(jaro),binary("AJaroWinklerFunction","rewrite.similarity.ajaro")(jaro),binary("AffineGapSimilarityFunction","rewrite.similarity.affine_gap")(affineGap),binary("EmailMatchTypeFunction","rewrite.similarity.email")(email),binary("PinCodeMatchTypeFunction","rewrite.similarity.pin")(pin),binary("OnlyAlphabetsExactSimilarity","rewrite.similarity.only_alpha_exact")(onlyAlphaExact),binary("OnlyAlphabetsAffineGapSimilarity","rewrite.similarity.only_alpha_affine")(onlyAlphaAffine),binary("SameFirstWordFunction","rewrite.similarity.same_first_word")(sameFirstWord))
  ...
  private val hashes:Seq[RewriteRule]=Seq(
    ...
    Seq(unary("blocking.lastWord","rewrite.blocking.lastWord")((c,_)=>javaLastWord(c)),unary("blocking.isNullOrEmpty","rewrite.blocking.isNullOrEmpty")((c,_)=>c.isNull||length(c.cast("string"))===0),unary("blocking.identityString","rewrite.blocking.identityString")((c,_)=>c), unary("blocking.identityInteger","rewrite.blocking.identityInteger")((c,_)=>c),unary("blocking.identityLong","rewrite.blocking.identityLong")((c,_)=>c), unary("blocking.identityBoolean","rewrite.blocking.identityBoolean")((c,_)=>c),unary("blocking.first2CharsBox","rewrite.blocking.first2CharsBox")((c,_)=>first2Box(c)), unary("blocking.first3CharsBox","rewrite.blocking.first3CharsBox")((c,_)=>first3Box(c))),
  ...
  private val misc:Seq[RewriteRule]=Seq(unary("preprocess.trim","rewrite.preprocess.trim")((c,_)=>trim(c)),unary("preprocess.caseNormalize","rewrite.preprocess.case_normalize")((c,_)=>lower(c.cast("string"))),unary("preprocess.stopWords","rewrite.preprocess.stop_words")((c,ctx)=>stopWords(c,ctx.parameters.getOrElse("pattern",throw new IllegalArgumentException("stopWords rewrite requires pattern")))),unary("model.vectorValue","rewrite.model.vector_value")((c,_)=>vectorValue(c)))
