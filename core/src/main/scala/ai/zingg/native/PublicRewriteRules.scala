...
object NativeExpressions {
  ...
  /** Java String/Zingg string algorithms operate on UTF-16 code units, while
    ...
    */
  private def utf16Hex(value: Column): Column = upper(
    hex(encode(value.cast("string"), "UTF-16BE"))
  )
  private def utf16Length(value: Column): Column =
    (length(utf16Hex(value)) / lit(4)).cast("int")
  private def indexArray(
      sizeValue: Column,
  ...
  private def decodeUtf16Hex(value: Column): Column =
    decode(unhex(value), "UTF-16BE")
  private def utf16Prefix(value: Column, count: Int): Column = {
    val h = utf16Hex(value); val n = utf16Length(value);
    val take = least(n, lit(count))
  ...
  def stringBase(left: Column, right: Column)(body: => Column): Column =
    when(
      left.isNull || right.isNull || length(
        left.cast("string")
      ) === 0 || length(right.cast("string")) === 0,
      lit(1.0)
    )
  ...
  def checkBlankOrNull(left: Column, right: Column): Column =
    when(
      left.isNotNull && right.isNotNull && length(
        left.cast("string")
      ) =!= 0 && length(right.cast("string")) =!= 0,
      lit(1.0)
    ).otherwise(lit(0.0))
  ...
  def integerSimilarity(left: Column, right: Column): Column = {
    ...
    val javaAbs =
      when(diff === lit(BigDecimal("-2147483648")), diff).otherwise(abs(diff))
    when(l.isNull || r.isNull || sum === lit(0), lit(0.0))
      .otherwise(lit(2.0) * javaAbs.cast("double") / sum.cast("double"))
  }
  ...
  def longSimilarity(left: Column, right: Column): Column = {
    ...
    val javaAbs = when(diff === lit(BigDecimal("-9223372036854775808")), diff)
      .otherwise(abs(diff))
    when(l.isNull || r.isNull || sum === lit(0), lit(0.0))
      .otherwise(lit(2.0) * javaAbs.cast("double") / sum.cast("double"))
  }

  def floatingSimilarity(
      left: Column,
      right: Column,
      dataType: String
  ): Column = {
    ...
    val difference =
      if (dataType == "float") abs((l - r).cast("float")).cast("double")
      else abs(l - r).cast("double")
    when(
      l.isNull || r.isNull || isnan(l.cast("double")) || isnan(
        r.cast("double")
      ),
      lit(1.0)
    )
      .otherwise(
        lit(1.0) - difference / (lit(1.0) + l.cast("double") + r.cast("double"))
      )
  }
  ...
  def arrayDoubleSimilarity(left: Column, right: Column): Column = {
    ...
    val dot = aggregate(
      zip_with(l, r, (a, b) => a * b),
      lit(0.0),
      (a, b) => a + coalesce(b, lit(0.0))
    )
    val ln = sqrt(aggregate(l, lit(0.0), (a, b) => a + b * b))
    val rn = sqrt(aggregate(r, lit(0.0), (a, b) => a + b * b))
    val cosine = abs(dot / (ln * rn))
    when(
      l.isNull || r.isNull || size(l) === 0 || size(
        r
      ) === 0 || !sameSize || hasNull,
      lit(0.0)
    )
      .when(ln > 0.0 && rn > 0.0, least(lit(1.0), cosine))
      .otherwise(lit(0.0))
  }

  /** SecondString SimpleTokenizer-compatible tokens: letter runs and digit
    * runs, case-insensitive.
    */
  def simpleTokens(value: Column): Column = array_distinct(
    regexp_extract_all(
      lower(value.cast("string")),
      lit("[\\p{L}]+|[\\p{N}]+"),
      lit(0)
    )
  )
  ...
  def jaccard(left: Column, right: Column): Column = stringBase(left, right) {
    ...
    when(union === 0, lit(0.0)).otherwise(
      size(array_intersect(l, r)).cast("double") / union.cast("double")
    )
  }

  def numbersJaccard(left: Column, right: Column): Column = {
    val l = array_distinct(
      regexp_extract_all(left.cast("string"), lit("[0-9]+"), lit(0))
    )
    val r = array_distinct(
      regexp_extract_all(right.cast("string"), lit("[0-9]+"), lit(0))
    )
    val union = size(array_union(l, r))
    when(
      left.isNull || right.isNull || length(
  ...
  private val ProductCodeRegex =
    "\\s[a-zA-Z]{0,4}\\s\\d+\\s([a-zA-Z]{0,4}\\s)?|(\\s?([a-z0-9A-Z]*\\d+(\\.\\d+)?[a-z0-9A-Z]*)\\s?)"
  def productCode(left: Column, right: Column): Column = {
    def codes(c: Column) = when(
      c.isNull || length(c.cast("string")) === 0,
      EmptyStringArray
    ).otherwise(
      array_distinct(
        transform(
          regexp_extract_all(c.cast("string"), lit(ProductCodeRegex), lit(0)),
          x => regexp_replace(x, " ", "")
        )
      )
    )
    val l = codes(left); val r = codes(right);
    val union = size(array_union(l, r)); val both = size(l) > 0 && size(r) > 0
  ...
  def jaro(left: Column, right: Column): Column = {
    ...
    val matches = aggregate(
      indexArray(size(chars1)),
      initialMatches,
      (state, i) => {
        ...
        val candidates = transform(
          filter(
            indexArray(windowLength),
            offset => {
              val j = start + offset
    ...
    val c1 = transform(
      matches.getField("sources"),
      i => element_at(chars1, i + lit(1))
    )
    val c2 = transform(
      array_sort(matches.getField("targets")),
      i => element_at(chars2, i + lit(1))
    )
    val count = size(c1)
    val mismatches = aggregate(
      zip_with(c1, c2, (a, b) => (a =!= b).cast("int")),
      lit(0),
      (a, b) => a + coalesce(b, lit(0))
    )
    val transpositions = floor(mismatches.cast("double") / lit(2.0))
    val score = (count.cast("double") / n.cast("double") + count.cast(
      "double"
    ) / m.cast("double") +
    ...
    stringBase(left, right) {
      when(chars1 === chars2, lit(1.0)).otherwise(
        when(size(c1) =!= size(c2) || count === 0, lit(0.0)).otherwise(score)
      )
    }
  }
  ...
  private def affineCharScore(aUnit: Column, bUnit: Column): Column = {
    ...
    val approximate = approximateGroups
      .map { group =>
  ...
  /** SecondString AffineGap/MongeElkan score expressed as higher-order Spark
    * SQL. State contains the previous M/S rows and the best score; the inner
    * fold builds the current M/S/T rows. No JVM row callback is used.
    */
  def affineGap(left: Column, right: Column): Column = {
    val a = utf16Units(left.cast("string"));
    val b = utf16Units(right.cast("string")); val n = size(a); val m = size(b)
    ...
    val initial =
      struct(zeros.alias("mrow"), zeros.alias("srow"), lit(0.0).alias("best"))
    val rows = aggregate(
      indexArray(n, oneBased = true),
      initial,
      (outer, i) => {
        val innerInit = struct(
          array(lit(0.0)).alias("mrow"),
          array(lit(0.0)).alias("srow"),
          array(lit(0.0)).alias("trow"),
          outer.getField("best").alias("best")
        )
        val row = aggregate(
          indexArray(m, oneBased = true),
          innerInit,
          (state, j) => {
            val score = affineCharScore(
              element_at(normalizedA, i),
              element_at(normalizedB, j)
            )
            ...
            val sv = greatest(
              lit(0.0),
              element_at(outer.getField("mrow"), j + lit(1)) - 5.0,
              element_at(outer.getField("srow"), j + lit(1)) - 1.0
            )
            val tv = greatest(
              lit(0.0),
              element_at(state.getField("mrow"), j) - 5.0,
              element_at(state.getField("trow"), j) - 1.0
            )
            struct(
              concat(state.getField("mrow"), array(mv)).alias("mrow"),
              concat(state.getField("srow"), array(sv)).alias("srow"),
              concat(state.getField("trow"), array(tv)).alias("trow"),
              greatest(state.getField("best"), mv, sv, tv).alias("best")
            )
          }
        )
        struct(
          row.getField("mrow").alias("mrow"),
          row.getField("srow").alias("srow"),
          row.getField("best").alias("best")
        )
      }
    )
    ...
    stringBase(left, right) {
      when(normalizedA === normalizedB, lit(1.0)).otherwise(
        when(denominator <= 0.0, lit(0.0)).otherwise(
          rows.getField("best") / denominator
        )
      )
    }
  }
  ...
  def pin(left: Column, right: Column): Column = {
    ...
    stringBase(left, right) {
      when(part(left) === part(right), lit(1.0)).otherwise(lit(0.0))
    }
  }
  def onlyAlphaExact(left: Column, right: Column): Column =
    stringBase(left, right) {
      when(
        regexp_replace(left.cast("string"), "[0-9.]", "") === regexp_replace(
          right.cast("string"),
          "[0-9.]",
          ""
        ),
  ...
  def onlyAlphaAffine(left: Column, right: Column): Column =
    stringBase(left, right) {
      affineGap(
        regexp_replace(left.cast("string"), "[0-9.]", ""),
        regexp_replace(right.cast("string"), "[0-9.]", "")
      )
    }
  def sameFirstWord(left: Column, right: Column): Column =
    stringBase(left, right) {
      affineGap(
        element_at(split(left.cast("string"), "-", -1), 1),
        element_at(split(right.cast("string"), "-", -1), 1)
      )
    }

  def javaLastWord(value: Column): Column = {
    val s = value.cast("string");
    val withoutTrailing = regexp_replace(s, " +$", "")
    when(s.isNull, s)
      .when(length(s) === 0, s)
      .when(length(withoutTrailing) === 0, lit(null).cast("string"))
  ...
  def first2Box(value: Column): Column = {
    val s = value.cast("string"); val p = utf16Prefix(s, 2);
    when(s.isNull || utf16Length(s) <= 2, 0)
      .when(p >= "aa" && p < "jz", 1)
      .when(p >= "jz" && p < "oz", 2)
      .when(p >= "oz", 3)
      .otherwise(4)
  }
  def first3Box(value: Column): Column = {
    val s = value.cast("string"); val p = utf16Prefix(s, 3);
    when(s.isNull || utf16Length(s) <= 3, 0)
      .when(p >= "aaa" && p < "ezz", 1)
      .when(p >= "ezz" && p < "izz", 2)
      .when(p >= "izz" && p < "mzz", 3)
      .when(p >= "mzz" && p < "qzz", 4)
  ...
  def truncate(value: Column, places: Int, dataType: String): Column = {
    val scale = math.pow(10, places); val scaled = value.cast("double") * scale;
    when(value.isNull, value).otherwise(
      (when(scaled >= 0.0, floor(scaled)).otherwise(ceil(scaled)) / scale)
        .cast(dataType)
    )
  }
  def trimDigits(value: Column, digits: Int, dataType: String): Column = {
    ...
    if (dataType == "int" || dataType == "long") {
      ...
      when(value.isNull, value).otherwise(
        when(q >= 0, floor(q)).otherwise(ceil(q)).cast(dataType)
      )
    } else
      when(value.isNull, value).otherwise(
        floor(value.cast("double") / lit(scale.toDouble)).cast(dataType)
      )
  }
  def range(value: Column, lo: Double, hi: Double, outType: String): Column =
    when(value.isNull, lit(0).cast(outType))
      .when(
        value.cast("double") >= lo && value.cast("double") < hi,
        lit(1).cast(outType)
      )
  ...
  def stopWords(value: Column, pattern: String): Column =
    when(value.isNull || lit(pattern).isNull, lit(null).cast("string"))
      .otherwise(regexp_replace(value.cast("string"), pattern, ""))
  ...
  def vectorValue(value: Column): Column = {
    ...
    val sparse = aggregate(
      zip_with(
        indices,
        values,
        (index, entry) => struct(index.alias("index"), entry.alias("value"))
      ),
      lit(0.0),
      (acc, pair) =>
        when(pair.getField("index") === lit(1), pair.getField("value"))
          .otherwise(acc)
    )
...
object PublicRewriteRules {
  ...
  private def binary(name: String, id: String)(f: (Column, Column) => Column) =
    new FunctionalRewriteRule(
      id,
      NativeOperation.resolve(s"similarity.$name"),
      (l, r, c) =>
        f(
          l,
          r.getOrElse(
            throw new IllegalArgumentException(s"$name requires right operand")
          )
        )
    )
  private def unary(op: String, id: String)(
      f: (Column, RewriteContext) => Column
  ) = new FunctionalRewriteRule(
    id,
    NativeOperation.resolve(op),
    (l, r, c) => f(l, c)
  )

  private val similarities: Seq[RewriteRule] = Seq(
    binary("SimilarityFunctionExact", "rewrite.similarity.exact")(exact),
    binary("StringSimilarityFunction", "rewrite.similarity.string_base")(
      (l, r) => stringBase(l, r) { lit(0.0) }
    ),
    binary("CheckNullFunction", "rewrite.similarity.check_null")(checkNull),
    binary(
      "CheckBlankOrNullFunction",
      "rewrite.similarity.check_blank_or_null"
    )(checkBlankOrNull),
    binary("IntegerSimilarityFunction", "rewrite.similarity.integer")(
      integerSimilarity
    ),
    binary("LongSimilarityFunction", "rewrite.similarity.long")(longSimilarity),
    binary("DoubleSimilarityFunction", "rewrite.similarity.double")((l, r) =>
      floatingSimilarity(l, r, "double")
    ),
    binary("FloatSimilarityFunction", "rewrite.similarity.float")((l, r) =>
      floatingSimilarity(l, r, "float")
    ),
    binary("DateSimilarityFunction", "rewrite.similarity.date")(dateSimilarity),
    binary("ArrayDoubleSimilarityFunction", "rewrite.similarity.array_double")(
      arrayDoubleSimilarity
    ),
    binary("JaccSimFunction", "rewrite.similarity.jaccard")(jaccard),
    binary("NumbersJaccardFunction", "rewrite.similarity.numbers_jaccard")(
      numbersJaccard
    ),
    binary("ProductCodeFunction", "rewrite.similarity.product_code")(
      productCode
    ),
    binary("JaroWinklerFunction", "rewrite.similarity.jaro")(jaro),
    binary("AJaroWinklerFunction", "rewrite.similarity.ajaro")(jaro),
    binary("AffineGapSimilarityFunction", "rewrite.similarity.affine_gap")(
      affineGap
    ),
    ...
    binary(
      ...
      "rewrite.similarity.only_alpha_exact"
    )(onlyAlphaExact),
    binary(
      "OnlyAlphabetsAffineGapSimilarity",
      "rewrite.similarity.only_alpha_affine"
    )(onlyAlphaAffine),
    binary("SameFirstWordFunction", "rewrite.similarity.same_first_word")(
      sameFirstWord
    )
  )
  ...
  private val hashes: Seq[RewriteRule] = Seq(
    (1 to 4).map(n =>
      unary(s"blocking.first${n}Chars", s"rewrite.blocking.first${n}Chars")(
        (c, _) => firstChars(c, n)
      )
    ),
    (1 to 3).map(n =>
      unary(s"blocking.last${n}Chars", s"rewrite.blocking.last${n}Chars")(
        (c, _) => lastChars(c, n)
      )
    ),
    Seq(
      unary("blocking.lastWord", "rewrite.blocking.lastWord")((c, _) =>
        javaLastWord(c)
      ),
      unary("blocking.isNullOrEmpty", "rewrite.blocking.isNullOrEmpty")(
        (c, _) => c.isNull || length(c.cast("string")) === 0
      ),
      unary("blocking.identityString", "rewrite.blocking.identityString")(
        (c, _) => c
      ),
      unary("blocking.identityInteger", "rewrite.blocking.identityInteger")(
        (c, _) => c
      ),
      unary("blocking.identityLong", "rewrite.blocking.identityLong")((c, _) =>
        c
      ),
      unary("blocking.identityBoolean", "rewrite.blocking.identityBoolean")(
        (c, _) => c
      ),
      unary("blocking.first2CharsBox", "rewrite.blocking.first2CharsBox")(
        (c, _) => first2Box(c)
      ),
      unary("blocking.first3CharsBox", "rewrite.blocking.first3CharsBox")(
        (c, _) => first3Box(c)
      )
    ),
    Seq("Dbl" -> "double", "Float" -> "float", "Int" -> "int", "Long" -> "long")
      .map { case (s, t) =>
        unary(s"blocking.lessThanZero$s", s"rewrite.blocking.lessThanZero$s")(
          (c, _) => when(c.isNull, lit(false)).otherwise(c.cast(t) < 0)
        )
      },
    Seq("Double" -> "double", "Float" -> "float").flatMap { case (s, t) =>
      (1 to 3).map(n =>
        unary(
          s"blocking.truncate${s}To${n}Places",
          s"rewrite.blocking.truncate${s}To${n}Places"
        )((c, _) => truncate(c, n, t))
      )
    },
    Seq("Dbl" -> "double", "Float" -> "float", "Int" -> "int", "Long" -> "long")
      .flatMap { case (s, t) =>
        (1 to 3).map(n =>
          unary(
            s"blocking.trimLast${n}Digits$s",
            s"rewrite.blocking.trimLast${n}Digits$s"
          )((c, _) => trimDigits(c, n, t))
        )
      },
    Seq("Dbl" -> "int", "Float" -> "int", "Int" -> "int", "Long" -> "long")
      .flatMap { case (s, t) =>
        Seq((0, 10), (10, 100), (100, 1000), (1000, 10000)).map {
          case (lo, hi) =>
            unary(
              s"blocking.rangeBetween${lo}And${hi}$s",
              s"rewrite.blocking.rangeBetween${lo}And${hi}$s"
            )((c, _) => range(c, lo, hi, t))
        }
      },
    Seq(
      unary("blocking.round", "rewrite.blocking.round")((c, _) => javaRound(c))
    )
  ).flatten

  private val misc: Seq[RewriteRule] = Seq(
    unary("preprocess.trim", "rewrite.preprocess.trim")((c, _) => trim(c)),
    unary("preprocess.caseNormalize", "rewrite.preprocess.case_normalize")(
      (c, _) => lower(c.cast("string"))
    ),
    unary("preprocess.stopWords", "rewrite.preprocess.stop_words")((c, ctx) =>
      stopWords(
        c,
        ctx.parameters.getOrElse(
          "pattern",
          throw new IllegalArgumentException(
            "stopWords rewrite requires pattern"
          )
        )
      )
    ),
    unary("model.vectorValue", "rewrite.model.vector_value")((c, _) =>
      vectorValue(c)
    )
  )
