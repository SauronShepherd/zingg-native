...
/** GraphFrames-compatible connected components using public relational Spark
  * APIs.
  */
object NativeGraph {
  private val Src = "src"
  ...
  private def maxValue(x: Column, y: Column): Column =
    when(x > y, x).otherwise(y)
  private def symmetrize(e: DataFrame): DataFrame = e
    .select(
      explode(
        array(
          struct(col(Src), col(Dst)),
          struct(col(Dst).alias(Src), col(Src).alias(Dst))
        )
      ).alias("edge")
    )
  ...
  def connectedComponents(
    ...
    require(
      sys.props
        .getOrElse("zingg.native.graph.strategy", "two_phase")
        .trim
        .toLowerCase == "two_phase",
      "zingg.native.graph.strategy must be two_phase"
    )
    val spark = vertices.sparkSession
    val root =
      s"${sys.props.getOrElse("zingg.native.graph.materializePath", "dbfs:/tmp/zingg-native-graph")}/${UUID.randomUUID()}"
    val ids = vertices.select(col(idColumn).alias("id")).distinct()
    val initial = edges
      .select(col(idColumn).alias(Src), col(rightIdColumn).alias(Dst))
      .unionByName(
        edges.select(col(rightIdColumn).alias(Src), col(idColumn).alias(Dst))
      )
      .filter(col(Src).isNotNull && col(Dst).isNotNull && col(Src) =!= col(Dst))
      .select(
        minValue(col(Src), col(Dst)).alias(Src),
        maxValue(col(Src), col(Dst)).alias(Dst)
      )
      .distinct()
    ...
    while (!converged && iteration <= maxIterations) {
      val large = ee
        .join(nbrs, Seq(Src))
        .select(col(Dst).alias(Src), col(MinNbr).alias(Dst))
        .distinct()
      val smallNbrs = large
        .groupBy(Src)
        .agg(min(col(Dst)).alias(MinNbr), count(lit(1)).alias(Count))
      val next = large
        ...
        .unionByName(
          smallNbrs.select(col(MinNbr).alias(Src), col(Src).alias(Dst))
        )
        .distinct()
    ...
    if (!converged)
      throw new NativeRewriteUnsupportedException(
        s"Connected-components did not converge within $maxIterations iterations"
      )
    val labels = ids
      .join(nbrs, ids("id") === nbrs(Src), "left")
      .select(
        ids("id"),
        coalesce(nbrs(MinNbr), ids("id")).alias("_component")
      )
    NativeEvidenceCollector.recordRule(
      context,
      "rewrite.graph.connected_components"
    )
    NativePlanGuard.guardDataFrame(
      vertices
        .join(labels, vertices(idColumn) === labels("id"), "left")
        .drop("id")
        .withColumnRenamed("_component", clusterColumn),
      context
    )
  }
}
