...
/** Runtime metadata only; Photon/native truth is supplied later by query
  * evidence.
  */
...
object Core {
  ...
  private def executionContext(
    ...
    val mode = sys.props
      .get("zingg.native.mode")
      .orElse(sys.env.get("ZINGG_NATIVE_MODE"))
      .getOrElse("STRICT")
    val runId = sys.props
      .get("zingg.native.run.id")
      .orElse(sys.env.get("ZINGG_NATIVE_RUN_ID"))
      .getOrElse(java.util.UUID.randomUUID().toString)
    val disabled = sys.props
      .get("zingg.native.disabled.rules")
      .orElse(sys.env.get("ZINGG_NATIVE_DISABLED_RULES"))
      .getOrElse("")
    RewriteContext(
      spark,
  ...
  /** Convenience gateway for one expression; production Zingg calls rewrite
  ...
  def transform(
    ...
    rewrite(
      df,
      normalizeOperation(operationId),
      left,
      Option(right),
      output,
  ...
  def rewrite(
      ...
      registry: RewriteRegistry = NativeRewriteRegistry.default
  ): DataFrame = {
    ...
    NativeExplain.emit(
      ...
      else if (context.mode.rewrites) "rewrite"
      else "passthrough"
    )
    if (disabled)
    ...
    if (!context.mode.rewrites) df
      ...
      NativePlanGuard.guardDataFrame(
        df.withColumn(output, rule(col(left), right.map(col), context)),
        context
      )
  }
  ...
  def rewriteColumns(
    ...
    val additions: Seq[(String, Column)] = operations.map {
      case (operationId, left, right, output) =>
        val normalized = normalizeOperation(operationId)
        val operation = NativeOperation.resolve(normalized)
        if (!registry.contains(operation)) {
          NativeExplain.emit(context, operation.id, None, "unsupported")
    ...
    NativeDiagnostics.planGuard(
      context,
      "start",
      s"operations=${operations.size}"
    )
  ...
  def preprocess(
    ...
    require(
      missing.isEmpty,
      s"Unknown preprocessing columns: ${missing.mkString(", ")}"
    )
    val normalized = normalizeOperation(operationId)
    val context = executionContext(df.sparkSession, "gateway.preprocess")
    columns.foldLeft(df)((current, field) =>
      rewrite(current, normalized, field, None, field, context)
    )
  }
}
