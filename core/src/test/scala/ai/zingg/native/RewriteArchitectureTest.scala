...
class RewriteArchitectureTest {
  @Test def parsesAllExecutionModes(): Unit = {
    assertEquals(
      ...
      NativeExecutionMode.parse("strict")
    )
    assertThrows(
      classOf[IllegalArgumentException],
      () => NativeExecutionMode.parse("fallback")
    )
  }
  ...
  @Test def registryRejectsDuplicateSemanticRules(): Unit = {
    val rule = new RewriteRule {
      ...
      def apply(
          left: org.apache.spark.sql.Column,
          right: Option[org.apache.spark.sql.Column],
          context: RewriteContext
      ) = left
    }
    assertThrows(
      classOf[IllegalArgumentException],
      () => RewriteRegistry(Seq(rule, rule))
    )
  }

  @Test def strictGuardFailsClosed(): Unit = {
    val report = NativeCompatibilityAnalyzer.analyze(
      "train",
      Seq((NativeOperation.resolve("similarity.jaro"), false, "callUDF"))
    )
    val context = RewriteContext(
      null,
      NativeExecutionMode.STRICT,
      RuntimeDescriptor("4.1.0", "2.13"),
      "train"
    )
    assertThrows(
      classOf[NativeRewriteUnsupportedException],
      () => NativePlanGuard.requireCompatible(report, context)
    )
  ...
  @Test def disabledRuleFailsClosedWithOperationAndRule(): Unit = {
    ...
    val failure = assertThrows(
      ...
      () =>
        Core.rewrite(
          null,
          "similarity.SimilarityFunctionExact",
          "left",
          None,
          "out",
          context
    ...
    assertTrue(
      failure.getMessage.contains("similarity.SimilarityFunctionExact")
    )
  }
  ...
  @Test def defaultRegistryContainsOnlyStablePublicRules(): Unit = {
    ...
    assertTrue(
      ids.forall(id => !id.contains("Catalyst") && !id.contains("SparkContext"))
    )
  }
}
