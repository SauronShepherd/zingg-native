...
def test_serverless_graph_benchmark_is_real_and_bounded() -> None:
    ...
    assert '\\"converged\\":$converged' in source
    assert "captureGraphIterations" in source
    assert "iterationEvidence.nonEmpty" in source
    assert "require(converged" in source
    assert "ThreadLocal[GraphIterationEvidence => Unit]" in diagnostics
    assert "graphIterationObserver.remove()" in diagnostics
